...
class Sprite extends Entity {
	...
	public function setScale(scaleWidth: Float = 1.0, scaleHeight: Float = 1.0): Void {			
		_sW = scaleWidth;
		_sH = scaleHeight;
	}
	...
	override public function render(canvas: Canvas): Void {
		...
		if (_image != null) {
			...
			canvas.g2.drawScaledSubImage(_image, Std.int(_animation.get() * _w) % _image.width, Math.floor(_animation.get() * _w / _image.width) * _h, _w, _h, (flip.x ? width:0), (flip.y ? height:0), (flip.x ? -width:width) * _sW, (flip.y ? -height:height) * _sH);
